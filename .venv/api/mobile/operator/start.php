...
$updateStmt = $pdo->prepare("UPDATE equipment SET status = 'working' WHERE id = ?");
$updateStmt->execute([$stationId]);

// Создаём заявку
$requestStmt = $pdo->prepare("
    INSERT INTO requests (type, description, employee_id, equipment_id, status_id)
    VALUES ('start', 'Запуск станка из мобильного приложения', ?, ?, 3)
");
$requestStmt->execute([$userId, $stationId]);

echo json_encode(['success' => true, 'message' => 'Станок запущен']);